...
void Configuration::read() {
    ...
    try {
        ...
        std::cerr << "Failed to parse YAML in file: " << file_name << std::endl
                  << e.what() << std::endl;
        return;
    }

    if (!root["configuration"] || !root["configuration"].IsMap()) {
        std::cerr << "Missing or invalid 'configuration' node in YAML." << std::endl;
        return;
    }

    const YAML::Node &configNode = root["configuration"];
    ...
    auto readPlayerConfig = [&](const string &playerKey, int *config_keys, string *device_keys) {
        if (!configNode[playerKey] || !configNode[playerKey].IsMap()) {
            std::cerr << "Missing or invalid '" << playerKey << "' node in 'configuration'." << std::endl;
            return;
        }

        const YAML::Node &playerNode = configNode[playerKey];

        int j = 0;
        for (int i = CONFIG_KEY_UP; i <= CONFIG_KEY_Z; i++) {
            const string &keyName = config_name[i];
            if (playerNode[keyName] && playerNode[keyName].IsMap()) {
                const YAML::Node &item = playerNode[keyName];
...
void Moves::initialize(string file_name) {
    ...
    if (!root["moves"] || !root["moves"]["move"] || !root["moves"]["move"].IsSequence()) {
        std::cerr << "Missing or invalid 'moves > move' array in YAML." << std::endl;
        return;
    }

    const YAML::Node &moveList = root["moves"]["move"];
...
void Player::initialize(string file_name, bool player_one, GLfloat x_pos, GLfloat y_pos)
{
    ...
    YAML::Node root_yaml = YAML::LoadFile(file_name);
    YAML::Node root = root_yaml["character"];
    if (!root) {
        std::cerr << "Missing root node 'character' in YAML file: " << file_name << std::endl;
        return;
    }
    ...
    // Actions
    YAML::Node actionNodes = asSequence(root["action"]);
    ...
    // Animations
    YAML::Node animations = asSequence(root["animation"]);
        for (const auto& anim : animations) {
            ...
            YAML::Node frames = asSequence(anim["frame"]);
            for (const auto& frame : frames) {
                ...
                // defense
                YAML::Node defs = asSequence(frame["defense"]);
                ...
                YAML::Node offenses = asSequence(frame["offense"]);
                ...
                YAML::Node create_objects = asSequence(frame["create_object"]);
                ...
                // collision
                YAML::Node collisions = asSequence(frame["collision"]);
